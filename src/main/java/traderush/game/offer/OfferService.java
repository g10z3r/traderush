...
public final class OfferService {
    ...
    public OfferService(
        ...
        this.offerRepository = Objects
                .requireNonNull(offerRepository, "offer repository cannot be null");
        ...
        return offerRepository.getById(offerId)
                .map(offer -> {
                    ...
                    if (!offer.getRewardRange().contains(rewardPerUnit)) {
                        return OfferOperationResult
                                .<TimedActiveOffer>failure(OfferError.REWARD_OUT_OF_RANGE);
                    }

                    if (!timedOffer.allowsDurationSeconds(durationSeconds)) {
                        return OfferOperationResult
                                .<TimedActiveOffer>failure(OfferError.DURATION_OUT_OF_RANGE);
                    }
                ...
                .orElseGet(() -> OfferOperationResult.failure(OfferError.OFFER_NOT_FOUND));
    }
        ...
        return offerRepository.getById(offerId)
                .map(offer -> {
                    if (!(offer instanceof LimitedOffer limitedOffer)) {
                        return OfferOperationResult
                                .<LimitedActiveOffer>failure(OfferError.WRONG_OFFER_TYPE);
                    }

                    if (!offer.getRewardRange().contains(rewardPerUnit)) {
                        return OfferOperationResult
                                .<LimitedActiveOffer>failure(OfferError.REWARD_OUT_OF_RANGE);
                    }
    ...
    public List<ActiveOffer> activeOffersForShop(ShopId shopId, long currentTick) {
